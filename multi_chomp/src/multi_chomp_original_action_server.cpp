...
//
// TIMING: result.computation_time measures set_paths() + the full solve
// loop, i.e. the algorithm's own work. It excludes the costmap wait, which
// is environmental latency, not optimizer time. result.iterations_executed
// records how many solve_step() calls actually ran (may be less than
// max_iterations if the plateau/convergence check triggers early). This is
// the per-window computation time for multi chomp: every call to this
// function is one full optimization window (the original coordinator only
// issues complete replans).
// ═══════════════════════════════════════════════════════════════════════════
    ...
    std::shared_ptr<GoalHandleMultiChomp> goal_handle)
{
  ...
  // ── Timing window starts here: covers set_paths + the solve loop ───────
  ...
  for (uint32_t iter = 0; iter < max_iter; ++iter)
  {
    ...
    if (iter % 10 == 0)
    {
      feedback->progress = static_cast<double>(iter) /
