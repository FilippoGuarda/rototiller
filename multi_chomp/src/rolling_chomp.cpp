...
void MultiChompNode::load_parameters()
{
    ...
    this->declare_parameter("anchor_weight", 10.0);
    ...
    params_.anchor_weight = this->get_parameter("anchor_weight").as_double();

    xidim_ = params_.num_robots * params_.waypoints_per_robot * cdim_;
    xi_ = VectorXd::Zero(xidim_);
    bb_ = VectorXd::Zero(xidim_);
}
...
void MultiChompNode::init_matrices()
{
    ...
    const double w_extra = params_.anchor_weight - 1.0;
    AA_.block(0, 0, cdim_, cdim_) += w_extra * MatrixXd::Identity(cdim_, cdim_);
    AA_.block(cdim_ * (nq - 1), cdim_ * (nq - 1), cdim_, cdim_) += w_extra * MatrixXd::Identity(cdim_, cdim_);
...
std::vector<nav_msgs::msg::Path> MultiChompNode::get_paths(const std::vector<nav_msgs::msg::Path> &templates) const
{
    ...
    if (xidim_ == 0 || xi_.size() != static_cast<int>(xidim_))
        return out;
...
void MultiChompNode::timer_callback()
{
    ...
    if (!optimization_active_ || xidim_ == 0 || xi_.size() == 0)
        return;
...
void MultiChompNode::solve_step()
{
    ...
    {
        size_t offset = static_cast<size_t>(r) * nq * cdim_;
        bb_.block(offset, 0, cdim_, 1) = params_.anchor_weight * start_states_[r];
        bb_.block(offset + (nq - 1) * cdim_, 0, cdim_, 1) = params_.anchor_weight * goal_states_[r];
    }

    VectorXd nabla_smooth = AAR_ * xi_ - bb_;
    ...
    for (int r = 0; r < params_.num_robots; ++r)
    {
        ...
        for (int i = 0; i < nq; ++i)
        {
            ...
            if (i == 0)
                qd = (xi_.block(idx + cdim_, 0, cdim_, 1) - start_p) / (2.0 * dt);
            else if (i == nq - 1)
                qd = (end_p - xi_.block(idx - cdim_, 0, cdim_, 1)) / (2.0 * dt);
            else
            ...
            if (i < 5)
                start_fade = static_cast<double>(i) / 5.0;
...
double MultiChompNode::compute_current_cost() const
{
    ...
    total_cost += params_.lambda * (0.5 * xi_.dot(AAR_ * xi_) - bb_.dot(xi_)); // CHANGED
