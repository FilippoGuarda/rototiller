    ...
    std::shared_ptr<const MultiChompOptimize::Goal> goal)
{
  if (goal->num_robots == 0 ||
      static_cast<int32_t>(goal->input_paths.size()) != goal->num_robots)
  {
...
// ═══════════════════════════════════════════════════════════════════════════
// execute_goal — rolling-window CHOMP update.
//
// TIMING: result.computation_time measures load_paths_into_state() + the
// solve loop + update_starts_from_tf() + export_state_to_paths(), i.e. the
// full per-cycle algorithm cost. It excludes the costmap wait. Full replans
// (fresh Nav2 initializations, max_iterations=100, set by the coordinator
// when new_plan_buffer is non-empty) and partial/sliding-window updates
// (max_iterations=10, issued every closed-loop cycle while a robot is
// moving) both go through this same execute_goal — this is the "rolling
// chomp window" computation time requested, covering both full and partial
// replans with a single measurement taken on every call.
// result.iterations_executed records how many solve_step() calls actually ran.
// ═══════════════════════════════════════════════════════════════════════════
    ...
    const std::shared_ptr<GoalHandleMultiChomp> goal_handle)
{
  ...
  auto result = std::make_shared<MultiChompOptimize::Result>();
  auto feedback = std::make_shared<MultiChompOptimize::Feedback>();
  ...
  // ── Timing window starts here ───────────────────────────────────────────
  const auto t_start = std::chrono::steady_clock::now();

  if (!load_paths_into_state(goal->input_paths))
  {
    result->computation_time = std::chrono::duration<double>(
  ...
  const double min_cost_change = 1;
  ...
  for (uint32_t iter = 0; iter < max_iter; ++iter)
  {
    if (goal_handle->is_canceling())
    {
      result->computation_time = std::chrono::duration<double>(
                                     std::chrono::steady_clock::now() - t_start)
                                     .count();
      result->iterations_executed = iterations_executed;
      goal_handle->canceled(result);
      cleanup_state();
      return;
    }
    ...
    if (iter % 10 == 0)
    {
      feedback->progress = static_cast<double>(iter) / static_cast<double>(max_iter);
