...
# ================================================================
#  E* implementation (static, LSM kernel) – from previous answer
# ================================================================
...
# ================================================================
#  Random connected graph + Dijkstra (as before)
# ================================================================
...
def estar_cost_matrix(
    ...
    """
    H, W = grid.shape
    obstacle = ~grid  # True where non-traversable
    risk = np.zeros_like(grid, dtype=float)  # uniform risk; plug yours if needed
    ...
    t0 = time.perf_counter()
    for j, s in enumerate(stations):
        gy, gx = s[1], s[0]  # note: grid indices (iy, ix) = (y, x)
...
def plot_results(sizes, t_dij, t_astar, t_estar):
    plt.figure()
    plt.plot(sizes, t_dij, marker="o",
             label="n nodes graph + Dijkstra (NetworkX)")
    plt.plot(sizes, t_astar, marker="s",
             label="10×n grid + A* (NetworkX)")
    plt.plot(sizes, t_estar, marker="^",
             label="10×n grid + E* (LSM)")
    ...
    plt.legend()
    plt.title("Cost-matrix computation time: Dijkstra vs A* vs E*, fixed number of robot-station pairs")
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    sizes, t_dij, t_astar, t_estar = run_benchmark()
    plot_results(sizes, t_dij, t_astar, t_estar)